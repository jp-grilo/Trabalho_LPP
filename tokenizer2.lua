...
local function CriaMetodos()
  local metodos = {}
  Iterador=Iterador+1
  local nomeMetodo = TabelaDeTokens[Iterador]["value"]
  Iterador=Iterador+1
  if TabelaDeTokens[Iterador]["value"] ~= "()" then
    Metodos["parametros"]=CriaParametros()
  end
  Iterador=Iterador+1
  if TabelaDeTokens[Iterador]["value"] == "vars" then
    Metodos["variaveis"]=CriaVariaveis()
  end
...
local function CriaClasse()
  ...
  return CorpoDaClasse
end



while Iterador < #TabelaDeTokens do
  if TabelaDeTokens[Iterador]["value"] == "class" then
    print(TabelaDeTokens[Iterador]["value"]) 
    Iterador=Iterador+1
    local Classes = {}
